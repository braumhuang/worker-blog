import type { NavigationItem, NavigationSection, NavigationTemplate, OptionMap } from '../types'

export const FIXED_NAVIGATION_IDS = ['home', 'memos', 'archives', 'categories', 'tags', 'links'] as const
...
export const DEFAULT_FIXED_NAVIGATION_ITEMS: NavigationItem[] = [
  { id: 'home', name: '首页', url: '/', visible: true, section: 'fixed', order: 10 },
  { id: 'memos', name: '闪念', url: '/memos', visible: true, section: 'fixed', order: 20 },
  { id: 'archives', name: '归档', url: '/archives', visible: true, section: 'fixed', order: 30 },
  { id: 'categories', name: '分类', url: '/categories', visible: false, section: 'fixed', order: 40 },
  { id: 'tags', name: '标签', url: '/tags', visible: true, section: 'fixed', order: 50 },
  { id: 'links', name: '友链', url: '/links', visible: true, section: 'fixed', order: 60 },
]

export const DEFAULT_CUSTOM_NAVIGATION_ITEMS: NavigationItem[] = [
  { id: 'about', name: '关于', url: '/post/about', visible: true, section: 'custom', template: 'about', order: 10 },
]
...
const FIXED_BY_ID = new Map(DEFAULT_FIXED_NAVIGATION_ITEMS.map((item) => [item.id, item]))
...
export function normalizeNavigationUrl(value: unknown, fallback = '/'): string {
  const input = String(value ?? '').trim().slice(0, 1000)
  if (!input) return fallback
  if (input.startsWith('/') && !input.startsWith('//') && !/[\s\\]/.test(input)) {
...
function navigationItemCompare(left: NavigationItem, right: NavigationItem): number {
...
function sortNavigationSections(fixed: NavigationItem[], custom: NavigationItem[]): NavigationItem[] {
  return [...fixed.sort(navigationItemCompare), ...custom.sort(navigationItemCompare)]
}
...
export function normalizeNavigationItems(raw: string | undefined): NavigationItem[] {
  if (!raw?.trim()) return cloneDefaults()

  let parsed: unknown
  try { parsed = JSON.parse(raw) } catch { return cloneDefaults() }
  ...
  for (const value of parsed.slice(0, 60)) {
    ...
    if (fallback) {
      ...
      fixed.push({
        id,
        name: cleanNavigationName(id === 'links' && source.name === '导航' ? '友链' : source.name, fallback.name),
        url: fallback.url,
      ...
      custom.push({
        id,
        name: cleanNavigationName(source.name, id === 'about' ? '关于' : '新菜单'),
        url: normalizeNavigationUrl(source.url, id === 'about' ? '/post/about' : '/'),
        visible: source.visible !== false,
        section: 'custom',
        template: normalizeTemplate(source.template ?? (id === 'about' ? 'about' : 'page')),
...
export function navigationItemsFromOptions(options: OptionMap): NavigationItem[] {
  return normalizeNavigationItems(options.navigation_menu)
}

export function navigationSectionsFromOptions(options: OptionMap): { fixed: NavigationItem[]; custom: NavigationItem[] } {
...
export function serializeNavigationItems(items: NavigationItem[]): string {
  return JSON.stringify(items.map(({ id, name, url, visible, section, template, order }) => ({
    id,
    name,
    url,
    visible,
    section,
    order,
    ...(section === 'custom' ? { template: template === 'about' ? 'about' : 'page' } : {}),
  })))
}
...
export function customNavigationItemForSlug(options: OptionMap, slug: string): NavigationItem | undefined {
  return navigationSectionsFromOptions(options).custom.find((item) => postSlugFromUrl(item.url) === slug)
}
