...
export async function createAdminSession(c: Context<AppEnv>): Promise<void> {
  ...
  await dbRun(c.env.BLOG_DB, 'INSERT INTO blog_cookies(cookie, expired) VALUES(?, ?)', token, expired)
...
export async function destroyAdminSession(c: Context<AppEnv>): Promise<void> {
  ...
  if (token) await dbRun(c.env.BLOG_DB, 'DELETE FROM blog_cookies WHERE cookie = ?', token)
  deleteCookie(c, SESSION_COOKIE, { path: '/', secure: new URL(c.req.url).protocol === 'https:' })
}

export function verifyCredentials(c: Context<AppEnv>, name: string, password: string): boolean {
  return constantTimeEqual(name, c.env.ADMIN_NAME) && constantTimeEqual(password, c.env.ADMIN_PSWD)
}
...
export const requireAdmin: MiddlewareHandler<AppEnv> = async (c, next) => {
  ...
  if (!row || row.expired <= now) {
    if (row) await dbRun(c.env.BLOG_DB, 'DELETE FROM blog_cookies WHERE cookie = ?', token)
    deleteCookie(c, SESSION_COOKIE, { path: '/', secure: new URL(c.req.url).protocol === 'https:' })
    return reject()
  }
  ...
  if (row.expired - now <= RENEW_BEFORE_SECONDS) {
    ...
    await dbRun(c.env.BLOG_DB, 'UPDATE blog_cookies SET expired = ? WHERE cookie = ?', renewed, token)
    setCookie(c, SESSION_COOKIE, token, cookieOptions(c))
  }
...
export const sameOriginOnly: MiddlewareHandler<AppEnv> = async (c, next) => {
  ...
  const validOrigin = origin ? origin === target.origin : Boolean(referer?.startsWith(`${target.origin}/`))
