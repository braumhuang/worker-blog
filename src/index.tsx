...
app.get('/uploads/*', async (c) => {
  let key = ''
  try { key = decodeURIComponent(c.req.path.slice('/uploads/'.length)) } catch { return c.text('Bad Request', 400) }
  ...
  if (rangeHeader) {
    ...
    if (!range) return c.body(null, 416, { 'Content-Range': `bytes */${head.size}` })
