...
import { customNavigationItemForSlug, navigationItemsFromOptions } from '../lib/navigation'
import { renderMarkdown } from '../lib/markdown'
import { excerptOf, isoDate, nowSeconds, positiveInt, resolveUploadedUrls } from '../lib/utils'
...
async function navigationCategories(db: D1Database, options: OptionMap): Promise<BlogMeta[]> {
  const enabled = navigationItemsFromOptions(options).some((item) => item.id === 'categories' && item.visible)
  if (!enabled) return []
  return dbAll<BlogMeta>(db, "SELECT * FROM blog_metas WHERE type='category' ORDER BY count DESC,name COLLATE NOCASE")
}
...
async function commentsForContent(c: Context<AppEnv>, content: BlogContent, options: OptionMap): Promise<CommentPageData> {
...
async function listByMeta(c: Context<AppEnv>, type: 'tag' | 'category', slug: string) {
  ...
  const totalPages = Math.max(1, Math.ceil(total / perPage))
  const safePage = Math.min(page, totalPages)
  const rows = await dbAll<BlogContent>(c.env.BLOG_DB, `SELECT c.* FROM blog_contents c JOIN blog_relationships r ON r.cid=c.cid WHERE r.mid=? AND c.type='post' AND c.status='publish' AND c.released<=? ORDER BY c.released DESC LIMIT ? OFFSET ?`, meta.mid, now, perPage, (safePage - 1) * perPage)
  const posts = await enrichContents(c.env.BLOG_DB, rows)
  const categories = await navigationCategories(c.env.BLOG_DB, options)
  return c.html(<Base options={options} title={meta.name} active={type === 'tag' ? 'tags' : 'categories'} categories={categories}>
    {type === 'tag'
      ? <Tag meta={meta} posts={posts} total={total} page={safePage} totalPages={totalPages} timeZone={options.site_timezone} fileCdnUrl={options.file_cdn_url}/>
      : <Category meta={meta} posts={posts} total={total} page={safePage} totalPages={totalPages} timeZone={options.site_timezone} fileCdnUrl={options.file_cdn_url}/>
    }
  </Base>)
}
...
function absoluteFeedHtml(html: string, origin: string): string {
  return html.replace(/(src|href|poster)=(['"])(\/(?!\/)[^'"<>]*)\2/g, (_match, attr, quote, path) => `${attr}=${quote}${origin}${path}${quote}`)
}

publicRoutes.get('/atom.xml', async (c) => {
  const options = await getOptions(c.env.BLOG_DB)
  const now = nowSeconds()
  const posts = await dbAll<BlogContent>(c.env.BLOG_DB, "SELECT * FROM blog_contents WHERE type='post' AND status='publish' AND released<=? ORDER BY released DESC, cid DESC LIMIT 20", now)
  const origin = new URL(c.req.url).origin
  const feedUrl = `${origin}/atom.xml`
  const homeUrl = `${origin}/`
  const updatedSeconds = posts.length ? Math.max(...posts.map((post) => Math.max(post.modified, post.released))) : now
  const entries = posts.map((post) => {
    const url = `${origin}/post/${encodeURIComponent(post.slug)}/`
    const html = absoluteFeedHtml(resolveUploadedUrls(renderMarkdown(post.text), options.file_cdn_url), origin)
  ...
  return c.body(xml, 200, { 'Content-Type': 'application/atom+xml; charset=utf-8', 'Cache-Control': 'public, max-age=300' })
})
...
publicRoutes.get('/post/:slug', (c) => c.redirect(`/post/${encodeURIComponent(c.req.param('slug'))}/`, 308))
...
publicRoutes.get('/', async (c) => {
  ...
  const safePage = Math.min(page, totalPages)
  const rows = await dbAll<BlogContent>(c.env.BLOG_DB, `SELECT * FROM blog_contents WHERE type='post' AND status='publish' AND released<=? ORDER BY released DESC LIMIT ? OFFSET ?`, now, perPage, (safePage - 1) * perPage)
  const posts = await enrichContents(c.env.BLOG_DB, rows)
  return c.html(<Base options={options} active="home" categories={await navigationCategories(c.env.BLOG_DB, options)}>
    <PostsPage posts={posts} timeZone={options.site_timezone} fileCdnUrl={options.file_cdn_url} page={safePage} totalPages={totalPages}/>
  </Base>)
})
...
publicRoutes.get('/post/:slug/', async (c) => {
  ...
  if (commentData && c.req.header('X-Requested-With') === 'comments') {
    return c.html(<Comments content={content} {...commentData} timeZone={options.site_timezone} saved={saved}/>)
  }
  return c.html(<Base options={options} title={content.title} active={navigationItem?.id} description={excerptOf(content.text, 150)} categories={await navigationCategories(c.env.BLOG_DB, options)}>
    {template === 'about' ? <About options={options} html={html}/> : template === 'page' ? <Page html={html}/> : <Post content={content} item={item} html={html} options={options}/>} 
    {commentData ? <Comments content={content} {...commentData} timeZone={options.site_timezone} saved={saved}/> : null}
  </Base>)
})
...
publicRoutes.post('/post/:slug/comments', async (c) => {
  ...
  if (c.req.header('X-Requested-With') === 'comments') {
    ...
    return c.html(<Comments content={content} {...commentData} timeZone={options.site_timezone} saved/>)
  }
...
publicRoutes.get('/memos/', async (c) => {
  ...
  const totalPages = Math.max(1, Math.ceil(total / perPage))
  const safePage = Math.min(page, totalPages)
  const memoRows = await dbAll<BlogContent>(c.env.BLOG_DB, "SELECT * FROM blog_contents WHERE type='memo' AND status='publish' AND released<=? ORDER BY released DESC LIMIT ? OFFSET ?", now, perPage, (safePage - 1) * perPage)
  ...
  return c.html(<Base options={options} title="闪念" active="memos" categories={await navigationCategories(c.env.BLOG_DB, options)}>
    <Memos memos={memos} days={days} total={total} page={safePage} totalPages={totalPages} timeZone={options.site_timezone} fileCdnUrl={options.file_cdn_url}/>
  </Base>)
})
...
publicRoutes.get('/archives/', async (c) => {
  ...
  const countRow = await dbFirst<{ total: number }>(c.env.BLOG_DB, `SELECT COUNT(*) AS total FROM blog_contents WHERE type='post' AND status='publish' AND released<=?`, now)
  const total = countRow?.total ?? 0
  const totalPages = Math.max(1, Math.ceil(total / perPage))
  const safePage = Math.min(page, totalPages)
  const posts = await dbAll<BlogContent>(c.env.BLOG_DB, `SELECT * FROM blog_contents WHERE type='post' AND status='publish' AND released<=? ORDER BY released DESC LIMIT ? OFFSET ?`, now, perPage, (safePage - 1) * perPage)
  ...
  return c.html(<Base options={options} title="归档" active="archives" categories={await navigationCategories(c.env.BLOG_DB, options)}>
    <Archives years={years} total={total} tagTotal={tagCount?.total ?? 0} page={safePage} totalPages={totalPages} timeZone={options.site_timezone}/>
  </Base>)
})

publicRoutes.get('/categories/', async (c) => {
  const options = await getOptions(c.env.BLOG_DB)
  const categories = await dbAll<BlogMeta>(c.env.BLOG_DB, "SELECT * FROM blog_metas WHERE type='category' ORDER BY count DESC,name COLLATE NOCASE")
  return c.html(<Base options={options} title="分类" active="categories" categories={categories}><Categories categories={categories}/></Base>)
})
...
publicRoutes.get('/tags/', async (c) => {
  ...
  return c.html(<Base options={options} title="标签" active="tags" categories={await navigationCategories(c.env.BLOG_DB, options)}><Tags tags={tags}/></Base>)
})
...
publicRoutes.get('/links/', async (c) => {
  ...
  return c.html(<Base options={options} title="导航" active="links" categories={await navigationCategories(c.env.BLOG_DB, options)}><Links links={links} fileCdnUrl={options.file_cdn_url}/></Base>)
})
