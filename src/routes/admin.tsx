...
import type { AppEnv, AttachmentInfo, AttachmentTemplate, BlogContent, BlogLink, BlogMeta, ContentStatus, ContentType, MetaType, NavigationItem, OptionMap } from '../types'
...
import { getOptions, normalizeFileCdnUrl, saveOptions, TIMEZONE_VALUES } from '../lib/options'
...
import { normalizeAttachmentTemplates, normalizeAttachmentTemplateType, serializeAttachmentTemplates } from '../lib/attachment-templates'
import {
  attachmentInfo, attachmentPath, draftSlug, formatDate, insertionForAttachment, intValue, nowSeconds,
  parseDatetimeLocal, positiveInt, publicAttachmentUrl, resolveUploadedUrls, safeReturnTo, slugify,
} from '../lib/utils'
...
function validContentType(value: string | undefined, fallback: ContentType = 'post'): ContentType {
  return ['post', 'page', 'atta', 'memo'].includes(value ?? '') ? value as ContentType : fallback
}
...
async function uniqueContentSlug(db: D1Database, type: ContentType, requested: string, cid: number): Promise<string> {
  ...
  const exists = (slug: string) => publicContent
...
adminRoutes.post('/admin/login', async (c) => {
  ...
  if (!verifyCredentials(c, name, password)) return c.html(<LoginPage error="用户名或密码错误" returnTo={returnTo} />, 401)
...
adminRoutes.get('/admin', async (c) => {
  ...
  const counts = await dbAll<{ type: ContentType; status: ContentStatus; total: number }>(c.env.BLOG_DB, "SELECT type, status, COUNT(*) AS total FROM blog_contents WHERE type != 'atta' GROUP BY type, status")
  ...
  return c.html(<DashboardPage options={options} counts={counts} recent={recent} links={links?.total ?? 0} comments={comments?.total ?? 0} />)
})

adminRoutes.get('/admin/navigation', async (c) => {
  const sections = navigationSectionsFromOptions(await getOptions(c.env.BLOG_DB))
  return c.html(<NavigationPage fixed={sections.fixed} custom={sections.custom} saved={Boolean(c.req.query('saved'))} />)
})

adminRoutes.post('/admin/navigation', async (c) => {
  const form = await c.req.formData()
  const ids = form.getAll('nav_id').map((value: FormDataEntryValue) => String(value).trim()).filter(Boolean).slice(0, 60)
  ...
  for (const id of ids) {
    ...
    submitted.push({
      ...
      url: normalizeNavigationUrl(form.get(`nav_url:${id}`), id === 'about' ? '/post/about' : '/'),
  ...
  await saveOptions(c.env.BLOG_DB, { navigation_menu: serializeNavigationItems(normalizeNavigationItems(JSON.stringify(submitted))) })
...
adminRoutes.get('/admin/contents', async (c) => {
  ...
  const pageOnly = type === 'post' && (requestedType === 'page' || c.req.query('filter') === 'page')
  ...
  return c.html(<ContentsPage options={options} type={type} pageOnly={pageOnly} statusFilter={statusFilter} page={page} total={total} perPage={perPage} rows={rows} now={nowSeconds()} />)
})
...
adminRoutes.get('/admin/content/new', async (c) => {
  ...
  const title = type === 'memo' ? formatDate(now, true, (await getOptions(c.env.BLOG_DB)).site_timezone) : ''
...
adminRoutes.get('/admin/content/:cid', async (c) => {
  ...
  return c.html(<ContentEditPage
    content={content} options={options} categories={categories}
    assignedCategoryIds={new Set(assigned.filter((meta) => meta.type === 'category').map((meta) => meta.mid))}
...
adminRoutes.post('/admin/content/:cid', async (c) => {
  ...
  const targetType: ContentType = current.type === 'memo' ? 'memo' : form.get('content_type') === 'page' ? 'page' : 'post'
  let title = String(form.get('title') ?? '').trim()
  if (targetType === 'memo') title = formatDate(released, true, options.site_timezone)
  if (!title) title = '未命名'
  const requestedSlug = targetType === 'memo' ? (current.slug.includes('-draft-') ? `memo-${released}` : current.slug) : String(form.get('slug') ?? '').trim() || title
  const slug = await uniqueContentSlug(c.env.BLOG_DB, targetType, requestedSlug, cid)
  const cover = targetType === 'memo' ? '' : String(form.get('cover') ?? '').trim()
  ...
  const categoryIds = targetType === 'memo' ? [] : [...new Set(form.getAll('categories').map((value: FormDataEntryValue) => intValue(value)).filter(Boolean))]
  ...
  await c.env.BLOG_DB.batch([
    c.env.BLOG_DB.prepare('UPDATE blog_contents SET title = ?, slug = ?, cover = ?, modified = ?, released = ?, text = ?, type = ?, status = ? WHERE cid = ?').bind(title, slug, cover, nowSeconds(), released, text, targetType, status, cid),
    c.env.BLOG_DB.prepare('DELETE FROM blog_relationships WHERE cid = ?').bind(cid),
    ...[...new Set([...categoryIds, ...tagIds])].map((mid) => c.env.BLOG_DB.prepare('INSERT OR IGNORE INTO blog_relationships(cid, mid) VALUES(?, ?)').bind(cid, mid)),
  ])
...
adminRoutes.post('/admin/content/:cid/delete', async (c) => {
  ...
  return c.redirect(content.type === 'memo' ? '/admin/contents?type=memo' : content.type === 'page' ? '/admin/contents?type=post&filter=page' : '/admin/contents?type=post')
})
...
adminRoutes.post('/admin/metas', async (c) => {
  ...
  } catch { return c.text('名称或别名已存在', 409) }
...
adminRoutes.post('/admin/metas/:mid/delete', async (c) => {
  ...
  await c.env.BLOG_DB.batch([c.env.BLOG_DB.prepare('DELETE FROM blog_relationships WHERE mid = ?').bind(mid), c.env.BLOG_DB.prepare('DELETE FROM blog_metas WHERE mid = ?').bind(mid)])
...
adminRoutes.get('/admin/attachments', async (c) => {
  ...
  return c.html(<AttachmentsPage rows={rows} fileCdnUrl={options.file_cdn_url} templates={normalizeAttachmentTemplates(options.attachment_templates)} page={page} total={total} perPage={perPage} />)
})
...
adminRoutes.get('/admin/attachment-templates', async (c) => {
  ...
  return c.html(<AttachmentTemplatesPage rows={rows} edit={rows.find((item) => item.id === editId) ?? null} saved={Boolean(c.req.query('saved'))} />)
})
...
adminRoutes.post('/admin/attachment-templates', async (c) => {
  ...
  const item: AttachmentTemplate = {
    ...
    template: String(form.get('template') ?? '').trim().slice(0, 10000),
  }
  if (!item.name || !item.template) return c.text('名称和模板不能为空', 400)
  const index = rows.findIndex((row) => row.id === id)
  if (index >= 0) rows[index] = item
  else rows.push(item)
  await saveOptions(c.env.BLOG_DB, { attachment_templates: serializeAttachmentTemplates(rows) })
...
adminRoutes.post('/admin/attachment-templates/:id/delete', async (c) => {
  const options = await getOptions(c.env.BLOG_DB)
  let id = ''
  try { id = decodeURIComponent(c.req.param('id')) } catch { return c.text('模板 ID 不正确', 400) }
  const rows = normalizeAttachmentTemplates(options.attachment_templates).filter((item) => item.id !== id)
  await saveOptions(c.env.BLOG_DB, { attachment_templates: serializeAttachmentTemplates(rows) })
...
adminRoutes.post('/admin/api/attachments', async (c) => {
  ...
  if (parent > 0 && !await dbFirst<{ cid: number }>(c.env.BLOG_DB, "SELECT cid FROM blog_contents WHERE cid = ? AND type != 'atta'", parent)) return c.json({ error: '关联内容不存在' }, 404)
  const ext = value.name.match(/(\.[a-zA-Z0-9]{1,10})$/)?.[1]?.toLowerCase() ?? ''
  const date = new Date()
  ...
  const options = await getOptions(c.env.BLOG_DB)
  const storedInfo: AttachmentInfo = { key, url: attachmentPath(key), mime, size: value.size, originalName: value.name }
  const now = nowSeconds()
  const result = await dbRun(c.env.BLOG_DB, "INSERT INTO blog_contents(parent, title, slug, created, modified, released, text, type, status) VALUES(?, ?, ?, ?, ?, ?, ?, 'atta', 'publish')", parent, value.name, key, now, now, now, JSON.stringify(storedInfo))
  ...
  return c.json({ attachment: { cid, parent, ...info, path: storedInfo.url }, insertion: insertionForAttachment(storedInfo) })
})
...
adminRoutes.get('/admin/comments', async (c) => {
  ...
  const total = count?.total ?? 0
  const rows = await dbAll<AdminCommentRow>(c.env.BLOG_DB, `SELECT cm.*, c.title AS content_title, c.slug AS content_slug, c.type AS content_type FROM blog_comments cm JOIN blog_contents c ON c.cid = cm.cid ${where} ORDER BY cm.created DESC, cm.id DESC LIMIT ? OFFSET ?`, ...args, perPage, (page - 1) * perPage)
  return c.html(<CommentsPage options={options} rows={rows} page={page} total={total} perPage={perPage} cid={cid} />)
})

adminRoutes.get('/admin/comment/:id', async (c) => {
  const id = intValue(c.req.param('id'))
  const options = await getOptions(c.env.BLOG_DB)
  const comment = await dbFirst<AdminCommentRow>(c.env.BLOG_DB, 'SELECT cm.*, c.title AS content_title, c.slug AS content_slug, c.type AS content_type FROM blog_comments cm JOIN blog_contents c ON c.cid = cm.cid WHERE cm.id = ? LIMIT 1', id)
  if (!comment) return c.notFound()
  return c.html(<CommentEditPage comment={comment} options={options} saved={Boolean(c.req.query('saved'))} />)
})
...
adminRoutes.post('/admin/comment/:id', async (c) => {
  ...
  if (site) {
    ...
    } catch { return c.text('网站地址格式不正确', 400) }
  }
...
adminRoutes.get('/admin/data/export', async (c) => {
  ...
  return c.body(JSON.stringify({ version: 6, exportedAt: new Date().toISOString(), tables: data }, null, 2), 200, { 'Content-Type': 'application/json; charset=utf-8', 'Content-Disposition': `attachment; filename="${filename}"`, 'Cache-Control': 'no-store' })
})
...
adminRoutes.post('/admin/data/import', async (c) => {
  ...
  try { payload = JSON.parse(await file.text()) as typeof payload } catch { return c.text('JSON 文件格式不正确', 400) }
  ...
  try {
    for (const spec of specs) {
      ...
      for (const row of rows) {
          ...
          if (normalizedRow.type === 'atta' && typeof normalizedRow.text === 'string') {
            try {
              ...
              if (parsedInfo && typeof parsedInfo === 'object' && !Array.isArray(parsedInfo)) {
                ...
                if (Object.prototype.hasOwnProperty.call(legacyInfo, 'parentCid')) { delete legacyInfo.parentCid; normalizedRow.text = JSON.stringify(legacyInfo) }
              }
            } catch { legacyParent = 0 }
          }
        ...
        if (spec.autoIncrement) await dbRun(c.env.BLOG_DB, `INSERT INTO ${spec.table}(${quoted}) VALUES(${placeholders})`, ...values)
        else {
    ...
    await dbRun(c.env.BLOG_DB, 'UPDATE blog_metas SET count = (SELECT COUNT(*) FROM blog_relationships r WHERE r.mid = blog_metas.mid)')
  } catch (error) {
    console.error(error)
...
adminRoutes.get('/admin/options', async (c) => {
  return c.html(<OptionsPage options={await getOptions(c.env.BLOG_DB)} saved={Boolean(c.req.query('saved'))} imported={c.req.query('imported')} />)
})
