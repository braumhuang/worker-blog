...
/**
 * Kehua · 留白 — Gridea Pro 主题样式
 * 移植自 kehuame/whitespace（Typecho），保持视觉 100% 一致并扩展 Gridea Pro 特有页面
 *
 * 章节：
 *  1. CSS 变量（亮 / 暗）
 *  2. 重置 / 基础
...
:root {
    ...
    --font-stack-body: 'PingFang SC', 'Microsoft YaHei', -apple-system, BlinkMacSystemFont, 'Helvetica Neue', Arial, sans-serif;
...
/* JS 注入的搜索结果列表（重置 ul 默认样式 + 让 anchor 占满整行） */
.search-result-list {
    list-style: none;
    margin: 0;
    padding: 0;
...
.article-category a { color: var(--color-text-muted); }
...
.article-tags .tag,
.tag-cloud .tag-cloud-item .tag-cloud-name {
    display: inline-block;
}
...
.memo-heatmap-cell[data-level="1"] { background-color: rgba(229, 57, 53, 0.25); }
.memo-heatmap-cell[data-level="2"] { background-color: rgba(229, 57, 53, 0.45); }
.memo-heatmap-cell[data-level="3"] { background-color: rgba(229, 57, 53, 0.7);  }
.memo-heatmap-cell[data-level="4"] { background-color: var(--color-primary);    }

.memo-heatmap-cell:hover {
    transform: scale(1.4);
}
...
.memo-content p { margin-bottom: var(--spacing-sm); }
...
.footer-theme a:hover { color: var(--color-primary); }
...
.back-to-top {
    ...
    visibility: hidden;
    transform: translateY(20px);
    transition: opacity var(--transition-normal), visibility var(--transition-normal), transform var(--transition-normal), border-color var(--transition-fast);
    z-index: 50;
}
...
@media (max-width: 480px) {
    ...
    .post-excerpt { font-size: 0.875rem; }
