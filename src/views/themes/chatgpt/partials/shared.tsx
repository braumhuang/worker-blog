...
export function Question({
  ...
  return (
    <div class="msg msg--user">
      <div
        class={`msg__bubble${title ? " msg__bubble--title" : ""}`}
        data-variants={variants}
        data-name={name}
        data-count={count === undefined ? undefined : String(count)}
      >
        {children}
      </div>
    </div>
    ...
    <article class="post-card">
      <a class="post-card__link" href={href}>
        <div class="post-card__body">
          <h3 class="post-card__title">{post.title || "未命名文章"}</h3>
          <p class="post-card__excerpt">{excerptOf(post.text)}</p>
...
export function PostCards({
  ...
  return posts.length ? (
    ...
    <div class="cg-empty">这里还没有内容。</div>
  );
}
...
export function Pagination({
  ...
  return (
    <nav class="pagination" aria-label="分页导航">
      {page > 1 ? (
        <a class="pagination__btn" href={href(page - 1)} rel="prev">
          ← 上一页
        </a>
      ) : (
        <span class="pagination__btn pagination__btn--disabled">← 上一页</span>
      )}
      <span class="pagination__info">
        第 {page} / {totalPages} 页
      </span>
      {page < totalPages ? (
        <a class="pagination__btn" href={href(page + 1)} rel="next">
          下一页 →
        </a>
      ) : (
        <span class="pagination__btn pagination__btn--disabled">下一页 →</span>
      )}
    </nav>
  );
}
...
export function MessageActions() {
  return (
    <div class="msg-actions">
      <button class="msg-action" id="copy-link" type="button" title="复制链接">
        <Icon name="copy" /> <span>链接</span>
      </button>
      <button class="msg-action" id="copy-text" type="button" title="复制全文">
        <Icon name="copy" /> <span>全文</span>
      </button>
      <button class="msg-action" id="go-top" type="button" title="返回顶部">
        <Icon name="up" /> <span>顶部</span>
      </button>
    </div>
  );
}
