...
function navIcon(id: string): IconName {
  ...
  if (id === "memos") return "memo";
  if (id === "tags") return "tag";
  if (id === "categories") return "category";
  if (id === "links") return "link";
...
export function Sidebar({
  ...
  const avatar = options.about_avatar
    ...
    : "";
  const initial =
    Array.from(options.site_title.trim())[0]?.toUpperCase() || "C";
  const aboutUrl =
    navigation.find((item) => item.id === "about")?.url || "/post/about/";
  const social = [
      ...
      <aside class="sidebar" id="sidebar">
        <div class="sidebar__top">
          <a
            ...
            href="/"
            aria-label={`${options.site_title} 首页`}
          >
            {avatar ? (
              <img
              ...
              <span class="sidebar__brand-logo sidebar__brand-logo--fallback">
                <Icon name="brand" />
              </span>
            )}
          </a>
        ...
        <button class="sidebar__search" id="sidebar-search" type="button">
          <Icon name="search" />
          <span>搜索</span>
        ...
        <nav class="sidebar__nav" aria-label="主导航">
          {navigation.map((item) => (
            <a
              class={`sidebar__nav-item${active === item.id ? " is-active" : ""}`}
              href={item.url}
              data-link={item.url}
            >
              <span class="sidebar__nav-icon">
                <Icon name={navIcon(item.id)} />
              </span>
        ...
        <div class="sidebar__recent">
          <div class="sidebar__recent-title">最近</div>
          <ul class="sidebar__recent-list" id="sidebar-recent-list">
            <li>
              <a href="/archives/">正在读取会话…</a>
            </li>
          </ul>
        </div>
        <div class="sidebar__foot">
          ...
          >
            {avatar ? (
              <img class="sidebar__user-avatar" src={avatar} alt="" />
            ) : (
              <span class="sidebar__user-avatar sidebar__user-avatar--fallback">
                {initial}
              </span>
            )}
            <span class="sidebar__user-meta">
              <span class="sidebar__user-name">{options.site_title}</span>
              <span class="sidebar__user-desc">{options.site_description}</span>
            </span>
            <Icon name="chevron" className="sidebar__user-chevron" />
          </button>
          <div class="user-menu" id="user-menu" hidden>
            <a class="user-menu__item" href={aboutUrl}>
              <Icon name="about" />
              <span>关于本站</span>
            </a>
        ...
        <div class="cg-sidebar-copyright">
