...
function CommentPagination({
  ...
  return (
    <nav class="cg-comment-pagination">
  ...
  return (
    <section class="comment-section" id="comments">
      <div class="comment-section__head">
        <Icon name="comment" />
        <span>继续这场对话 · {total} 条评论</span>
      </div>
      {saved ? <div class="cg-comment-notice">评论已提交。</div> : null}
      <form
        class="cg-comment-form"
        method="post"
        action={`/post/${encodeURIComponent(content.slug)}/comments`}
      >
        <div class="cg-comment-fields">
          <input
            name="name"
            maxLength={100}
            placeholder="名字 *"
      ...
      <div class="cg-comments-list">
        {comments.length ? (
          comments.map((comment) => (
            <article class="cg-comment">
              <div class="cg-comment-avatar">
          ...
          <div class="cg-comments-empty">还没有评论，来开启下一轮对话吧。</div>
        )}
      </div>
      <CommentPagination
        page={page}
