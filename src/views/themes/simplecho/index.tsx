...
export function Posts({ posts, timeZone, fileCdnUrl }: { posts: ContentWithMeta[]; timeZone: string; fileCdnUrl: string }) {
  if (!posts.length) return <div class="sc-empty"><p>笔尖还在等纸——尚无文章。</p></div>;
  return <div class="sc-post-inner">{posts.map((post) => <PostCard post={post} timeZone={timeZone} fileCdnUrl={fileCdnUrl} />)}</div>;
}
...
export function Index({
  ...
  return (
    <div class="sc-post-list-container">
      {page === 1 && options?.site_description ? <div class="sc-post-inner"><div class="sc-sticky-card">📌 {options.site_description}</div></div> : null}
