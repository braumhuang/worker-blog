  ...
  return (
    <section class="sc-comments" id="comments">
      <div class="sc-comments-card">
        <h2 class="sc-comments-title">评论 <span>{total}</span></h2>
        {saved ? <div class="sc-comment-notice">评论已提交。</div> : null}
        <form class="sc-comment-form" method="post" action={`/post/${encodeURIComponent(content.slug)}/comments`}>
          ...
          <textarea name="text" maxLength={5000} placeholder="评论内容 *" required />
          <div class="sc-comment-actions"><button type="submit">发表评论</button></div>
        </form>
        <div class="sc-comments-list">
          {comments.length ? comments.map((comment) => (
            <article class="sc-comment">
              <div class="sc-comment-avatar">{Array.from(comment.name.trim())[0]?.toUpperCase() || "?"}</div>
          ...
          )) : <div class="sc-empty sc-comments-empty">还没有评论，来说两句吧。</div>}
        </div>
