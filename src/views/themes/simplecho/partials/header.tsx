...
export function Header({
  ...
  const avatar = options.about_avatar
    ...
    : "";
  const initial =
    Array.from(options.site_title.trim())[0]?.toUpperCase() || "S";
  return (
    <nav class="sc-navbar" role="navigation" aria-label="主导航">
      <a
        class="sc-navbar-brand"
        href="/"
      ...
      >
        {avatar ? (
          ...
          <span class="sc-avatar sc-avatar-fallback" aria-hidden="true">
            {initial}
          </span>
        )}
        <div>
          ...
          {options.site_description ? (
            <div class="sc-site-sub">{options.site_description}</div>
          ) : null}
        </div>
      </a>

      <button
        class="sc-navbar-toggler"
        type="button"
        id="sc-navbar-toggler"
        aria-controls="sc-navbar-collapse"
        aria-expanded="false"
        aria-label="切换导航"
      >
        <Icon name="menu" />
      ...
      <div class="sc-navbar-collapse" id="sc-navbar-collapse">
        ...
        <div class="sc-palette-switcher" role="group" aria-label="切换配色">
          <button
            type="button"
            class="sc-palette-dot is-gray"
            data-sc-set-palette="gray"
            aria-label="银光灰"
            title="银光灰"
          />
          <button
            type="button"
            class="sc-palette-dot is-white"
            data-sc-set-palette="white"
            aria-label="简约白"
            title="简约白"
          />
          <button
            type="button"
            class="sc-palette-dot is-green"
            data-sc-set-palette="green"
            aria-label="墨草绿"
            title="墨草绿"
          />
          <button
            type="button"
            class="sc-palette-dot is-black"
            data-sc-set-palette="black"
            aria-label="暗夜黑"
            title="暗夜黑"
          />
        </div>
        ...
        >
          ...
          />
          <div
            class="sc-search-results"
            id="sc-search-results"
            role="listbox"
          />
        </form>
      </div>
