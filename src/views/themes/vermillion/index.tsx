...
import { Divider, magazineDate, magazineTime, Masthead, Seal } from "./partials/shared";
...
export function Posts({ posts, timeZone, fileCdnUrl }: { posts: ContentWithMeta[]; timeZone: string; fileCdnUrl: string }) {
  ...
  return (
    <section class="grid-earlier" style="margin-top:36px">
      {posts.map((post, index) => <PostCard post={post} timeZone={timeZone} fileCdnUrl={fileCdnUrl} index={index} />)}
    </section>
  );
}
  ...
  return (
    <>
      ...
      <article class="hero-bill fade-in" style="animation-delay:0.2s">
        <div>
          <div class="hero-eyebrow"><span>EDITORIAL · 卷首</span><span class="sub">今日 · 综述</span></div>
          <div class="hero-prose">
            笔耕不辍，纸自留香。把每一次小小的回响都写下来——多年后再翻，那一页都还在原处。或许慢就是最稳的姿态：<em>让事情自己长成它该有的样子</em>。
          </div>
          {tags.length ? (
            <div class="hero-tags">
              {tags.slice(0, 3).map((tag) => <a class="wtag" href={`/tag/${encodeURIComponent(tag.slug)}/`}>{tag.name}</a>)}
            </div>
          ) : null}
        </div>
        <div>
          ...
          <div class="margin-note">A periodical of small presences — kept on paper, returned to slowly.<span class="zh">所有小小的当下，都被纸记住。</span></div>
        </div>
      </article>
      ...
      {earlier.length ? (
        <>
          <Divider glyph="ii" delay="0.9s">更早 · Earlier selves</Divider>
          <section class="grid-earlier">
            {earlier.map((post, index) => <PostCard post={post} timeZone={timeZone} fileCdnUrl={fileCdnUrl} index={index + 5} eye="Earlier · 更早" />)}
          </section>
        </>
      ) : null}
