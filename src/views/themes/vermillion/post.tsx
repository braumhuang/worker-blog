  ...
  return (
    <>
      <Masthead
        ...
        date={formatDate(content.released, false, options.site_timezone).replaceAll("-", " · ")}
        extra={`约 ${readingMinutes(content.text)} min · ${wordCount(content.text)} 字`}
      />
      <div class="reading-progress" data-reading-progress><div class="reading-progress-bar" /></div>
      <article class="post-article">
        <div>
          {content.cover ? (
            <div class="post-feature-img"><img src={publicAttachmentUrl(content.cover, options.file_cdn_url)} alt={content.title} /></div>
          ) : null}
          <div class="post-meta-row">
            <span class="vermillion">{formatDate(content.released, false, options.site_timezone)}</span>
            ...
            {content.modified > content.released ? <span>· 修订 {formatDate(content.modified, false, options.site_timezone)}</span> : null}
          </div>
          <h1 class="post-title-main">{content.title}</h1>
          {item.tags?.length ? (
            <div class="hero-tags" style="margin-top:0;margin-bottom:32px">
              {item.tags.map((tag) => <a class="wtag" href={`/tag/${encodeURIComponent(tag.slug)}/`}>{tag.name}</a>)}
            </div>
          ) : null}
          <div class="prose post-content" data-toc data-code-copy dangerouslySetInnerHTML={{ __html: html }} />
        </div>
