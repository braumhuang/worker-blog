...
export function CommentEditPage({ comment, options, saved }: { comment: AdminCommentRow; options: OptionMap; saved: boolean }) {
  return <AdminLayout title="编辑评论" subtitle={`评论 #${comment.id}`} actions={<a class="button" href="/admin/comments">返回列表</a>}>
    {saved ? <div class="notice">评论已保存。</div> : null}<section class="panel"><div class="panel-body"><form method="post" action={`/admin/comment/${comment.id}`} class="main-form comment-edit-form">
      ...
      <div class="field"><label>网站</label><input class="input" name="site" maxLength={500} value={comment.site} /></div><div class="field"><label>评论内容</label><textarea class="textarea comment-edit-text" name="text" maxLength={5000} required>{comment.text}</textarea></div>
