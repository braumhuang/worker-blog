import type { AttachmentTemplate, BlogContent, BlogMeta, OptionMap } from '../../types'
...
export function ContentEditPage({ content, options, categories, assignedCategoryIds, assignedTags, attachments, templates, saved }: {
  content: BlogContent
  options: OptionMap
  categories: BlogMeta[]
  assignedCategoryIds: Set<number>
  assignedTags: BlogMeta[]
  attachments: Array<{ content: BlogContent; info: import('../../types').AttachmentInfo }>
  templates: AttachmentTemplate[]
  saved: boolean
}) {
  const label = content.type === 'memo' ? '闪念' : content.type === 'page' ? '页面' : '文章'
  const returnType = content.type === 'memo' ? 'memo' : 'post'
  return <AdminLayout title={`编辑${label}`} subtitle={`CID ${content.cid}`} actions={<a class="button" href={`/admin/contents?type=${returnType}${content.type === 'page' ? '&filter=page' : ''}`}>返回列表</a>}>
    {saved ? <div class="notice">保存成功。</div> : null}
    <form method="post" action={`/admin/content/${content.cid}`} class="form-grid">
      <div class="main-form">
        {content.type !== 'memo' ? <>
          ...
          <div class="field cover-url-field"><label for="cover">封面 URL</label><div class="input-inline-action"><input class="input" id="cover" name="cover" type="text" inputMode="url" value={content.cover || ''} placeholder="https://example.com/cover.jpg 或 /2026/07/cover.jpg" data-cover-url /><label class="input-inline-button" for="cover-upload">上传</label><input id="cover-upload" type="file" accept="image/*" data-cover-upload data-cid={content.cid} hidden /></div><small class="muted" data-cover-status></small></div>
        </> : <div class="memo-editor-note">闪念标题会根据发布时间自动生成，格式为“{formatDate(content.released, true, options.site_timezone)}”。</div>}
        <div class="editor-panel" data-editor-panel><EditorToolbar /><div class="editor-workspace"><textarea class="editor-textarea" name="text" data-editor>{content.text}</textarea><div class="editor-preview prose-preview" data-preview></div></div></div>
      </div>
      <aside class="side-form">
        <section class="side-box"><h3>发布</h3><div class="side-box-body">
          {content.type !== 'memo' ? <div class="field"><label for="content_type">类型</label><select class="select" id="content_type" name="content_type"><option value="post" selected={content.type === 'post'}>文章</option><option value="page" selected={content.type === 'page'}>页面</option></select></div> : null}
