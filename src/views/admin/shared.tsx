import type { AttachmentInfo, AttachmentTemplate, BlogContent, NavigationItem, NavigationSection } from '../../types'
...
export function EditorToolbar() {
  const buttons = [
    ...
    ['a', 'A', 'A标签'], ['image', '▧', '图片'], ['more', '…', '摘要分隔'], ['hr', '—', '分割线'],
  ]
...
export function NavigationRows({ items, section }: { items: NavigationItem[]; section: NavigationSection }) {
  return <div class="navigation-list" data-navigation-list data-navigation-section={section}>{items.map((item) => {
    ...
    return <div class={`navigation-row navigation-row-${section}`} data-navigation-row data-navigation-id={item.id}>
      <input type="hidden" name="nav_id" value={item.id} /><input type="hidden" name={`nav_section:${item.id}`} value={section} />
      ...
      <div class="field navigation-url-field"><label>页面 URL</label><input class="input" name={`nav_url:${item.id}`} value={item.url} maxLength={1000} readOnly={section === 'fixed'} required /></div>
...
export function AttachmentRows({ rows, fileCdnUrl, templates }: { rows: Array<{ content: BlogContent; info: AttachmentInfo }>; fileCdnUrl: string; templates: AttachmentTemplate[] }) {
  return <div class="attachment-list" data-attachment-list data-attachment-templates={JSON.stringify(templates)}>{rows.map(({ content, info }) => {
    ...
    return <div class="attachment-item">
      ...
      <div><button type="button" class="button small" data-attachment-insert data-attachment-path={info.url} data-attachment-name={info.originalName} data-attachment-kind={templateType}>插入</button> <button type="button" class="button small danger" data-attachment-delete={content.cid}>删除</button></div>
    </div>
  })}</div>
}
