  ...
  return <html lang="zh-CN">
    ...
    <body>
      <header class="admin-topbar"><div class="admin-topbar-inner">
        <nav class="admin-desktop-nav" aria-label="后台主导航">{adminNav.map(([href, label]) => <a class={activeNav(title, label) ? 'active' : undefined} href={href}>{label}</a>)}</nav>
        <div class="admin-user"><a href="/" target="_blank">查看站点</a><a href="/admin/logout">退出</a></div>
        <details class="admin-mobile-menu"><summary>菜单</summary><div class="admin-mobile-panel">
      ...
      <main class="admin-shell">
        <header class="admin-heading"><div class="admin-heading-copy"><h1>{title}</h1>{subtitle ? <p>{subtitle}</p> : null}</div>{actions ? <div class="admin-heading-actions">{actions}</div> : null}</header>
        {children}
      </main>
...
export function AdminPagination({ page, totalPages, path }: { page: number; totalPages: number; path: string }) {
  if (totalPages <= 1) return null
  const separator = path.includes('?') ? '&' : '?'
  const items = Array.from({ length: totalPages }, (_, index) => index + 1).filter((n) => n === 1 || n === totalPages || Math.abs(n - page) <= 2)
  let previous = 0
  return <nav class="pagination-admin">{items.map((n) => {
    const gap = previous && n - previous > 1
    previous = n
    return <>{gap ? <span>…</span> : null}{n === page ? <span>{n}</span> : <a href={`${path}${separator}page=${n}`}>{n}</a>}</>
  })}</nav>
}
