...
缓存规范化后的站点设置，最长 5 分钟。保存站点设置、导航、附件模板或导入数据后会删除缓存。

### sessions_cache

使用后台会话令牌的 SHA-256 摘要作为缓存键，缓存会话查询结果，最长 5 分钟且不会超过会话剩余有效期。

后台会话默认有效期为 10 天，剩余 2 天时自动续期。登录、续期、退出和过期处理会同步更新缓存与 D1。

Cache API 不可用、数据无效或缓存未命中时，系统自动回退到 D1。
