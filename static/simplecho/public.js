(function () {
  ...
  function applyPalette(palette) {
    ...
    $$("[data-sc-set-palette]").forEach(function (button) {
      button.setAttribute(
        "aria-pressed",
        button.getAttribute("data-sc-set-palette") === palette
          ? "true"
          : "false",
      );
    });
  ...
  function initPalette() {
    applyPalette(
      document.documentElement.getAttribute("data-sc-palette") || "gray",
    );
    $$("[data-sc-set-palette]").forEach(function (button) {
  ...
  function initActiveMenu() {
    ...
    $$(".sc-nav-item a").forEach(function (link) {
      ...
      if (
        href === current ||
        (href !== "/" && current.indexOf(href + "/") === 0)
      )
        link.closest(".sc-nav-item").classList.add("is-active");
    });
  }
  ...
  function initReadingProgress() {
    ...
    var update = function () {
      ...
      var distance = Math.max(
        1,
        content.offsetHeight - window.innerHeight * 0.3,
      );
      var progress = Math.max(
        0,
        Math.min(1, (scrollTop - start + window.innerHeight * 0.25) / distance),
      );
  ...
  function initBackToTop() {
    ...
    if (!button) return;
    var update = function () {
      button.classList.toggle("is-visible", (window.scrollY || 0) > 400);
    };
    update();
    addEventListener("scroll", update, { passive: true });
    button.addEventListener("click", function () {
        ...
        .then(function (data) {
          ...
          results.innerHTML = items.length
            ? items
                .map(function (item) {
                  return (
                    '<a class="sc-search-result-item" href="' +
                    escapeHtml(item.url || "#") +
                    '"><strong>' +
                ...
                .join("")
            : '<div class="sc-search-result-item sc-search-empty">没有找到匹配内容</div>';
          results.classList.add("is-open");
        })
        .catch(function (error) {
          if (error && error.name === "AbortError") return;
          results.innerHTML =
            '<div class="sc-search-result-item sc-search-empty">搜索暂时不可用</div>';
          results.classList.add("is-open");
        });
    };
