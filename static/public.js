...
(() => {
  ...
  const closeMobileSubmenus = (except = null, scope = nav) => {
    Array.from(scope?.children || []).forEach((group) => {
      if (!group.matches?.('[data-mobile-nav-group]') || group === except) return;
      group.classList.remove('open');
      group.querySelector(':scope > [data-mobile-nav-toggle]')?.setAttribute('aria-expanded', 'false');
    });
  };
  ...
  document.addEventListener('keydown', (event) => {
    ...
    if (event.key === 'Escape') { setSearchOpen(false); setMenuOpen(false); setMoreOpen(false); }
  ...
  const replaceComments = async (response, { historyUrl = '', historyMode = '', scrollMode = 'keep' } = {}) => {
    if (!response.ok) throw new Error(await response.text() || '评论操作失败');
    ...
    if (historyUrl && historyMode === 'push') history.pushState({}, '', historyUrl);
    if (historyUrl && historyMode === 'replace') history.replaceState({}, '', historyUrl);
  ...
  addEventListener('popstate', async () => {
    ...
    try { await replaceComments(await fetchComments(location.href), { scrollMode: 'keep' }); }
